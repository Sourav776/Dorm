...
public partial class Admin_Review : System.Web.UI.Page
{
        ...
        using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["DormitoryConnectionString"].ToString()))
            try
            {
                ...
                string query = "select * from StudentInfo A Left join Users B ON A.UserName=B.UserName AND" +
                    " A.UserType=B.UserType where A.DeptStatus=2 AND A.HallStatus=2 AND A.ATime IS NULL " +
                    "order by A.ID,A.DTime,A.HTime";
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        ...
        con.Open();
        string query = "UPDATE StudentInfo SET  AdminStatus=@Status,ATime =@Time WHERE ID=@ID";
