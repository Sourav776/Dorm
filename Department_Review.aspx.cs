...
public partial class Department_Review : System.Web.UI.Page
{
    ...
    protected void func()
    {
        using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["DormitoryConnectionString"].ToString()))
            try
            {
                con.Open();
                SqlDataAdapter da;
                string query = "select * from StudentInfo A Left join Users  B ON A.UserName=B.UserName" +
                    " AND A.UserType=B.UserType where A.DeptStatus = 1 AND A.DTime IS NULL order by A.ID";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
       ...
       // Response.Write(pos + "</br>");
        //Response.Write(table.Rows.Count + "</br>");
    }
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        ...
        con.Open();
        string query = "UPDATE StudentInfo SET  DeptStatus=@Status,DTime =@Time WHERE ID=@ID";
